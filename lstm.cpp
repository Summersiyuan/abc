...
//	PRINT("%d\n", i2t_gap);
//	PRINT("%d\n", i_gap);
	int rem=(data_size+1)%(batch_size);
	printf("rem=%d\n", rem);
	for(epoch=1; epoch<ITERATION_NUM+1; epoch++)  //训练次数
	{
		//printf("%d\n", batch_size); 
	    for(l=rem; l<data_size-3*(batch_size)+1; l++)
		{
			a_int=0;
			b_int=0;
			c_int=0;
           ...
           //保存每次生成的预测值
			...
			{
				a_int += flavornum_day[l+m+1][flavortype];  //输入-3, -2, -1 的数据
				PRINT("a1--%d\n", l+m+1);
			}
			...
			for(int m=0; m<batch_size; m++)
			{
				b_int += flavornum_day[l+batch_size+m+1][flavortype];  //输入-2,-1,0 的数据
				PRINT("b1--%d\n", l+batch_size+m+1);
			}
			...
			for(int m=0; m<batch_size; m++)
			{
				c_int += flavornum_day[l+2*(batch_size)+m+1][flavortype];            //输入-1, 0, 1的数据
				PRINT("c1--%d\n", l+2*(batch_size)+m+1);
			}
...
<<<<<<< HEAD
   ...
   //保存每次生成的预测值
	...
	for(int m=0; m<batch_size; m++)
	{
		a_int += flavornum_day[start_f+m-batch_size+1][flavortype];  //输入-3, -2, -1 的数据
		PRINT("a--%d\n", start_f+m-batch_size+1);
	}
	...
	for(int m=0; m<batch_size; m++)
	{
		b_int += flavornum_day[start_f+m+1][flavortype];  //输入-2,-1,0 的数据
		PRINT("b--%d\n", start_f+m+1);
	}
	int2binary(b_int, b);                 //转为二进制数
	...
	if(flavor[flavortype][0]<0)
		flavor[flavortype][0]=0;
